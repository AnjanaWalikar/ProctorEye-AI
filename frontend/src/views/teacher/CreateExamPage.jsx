...
const examValidationSchema = yup.object({
  ...
  examType: yup.string().oneOf(['mcq', 'coding', 'both'], 'Invalid exam type').required('Exam Type is required'),
  totalQuestions: yup
    .number()
  ...
  codingQuestion: yup.object({
    question: yup.string().when('examType', {
      is: (type) => type === 'coding' || type === 'both',
      then: (schema) => schema.required('Coding Question is required'),
      otherwise: (schema) => schema.nullable(),
    }),
    description: yup.string().when('examType', {
      is: (type) => type === 'coding' || type === 'both',
      then: (schema) => schema.required('Question Description is required'),
      otherwise: (schema) => schema.nullable(),
    }),
  }),
});
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (examResponse) {
        ...
        // Only create coding question if examType is 'coding' or 'both'
        if (values.examType === 'coding' || values.examType === 'both') {
          ...
          try {
            ...
            if (codingResponse.data.success) {
              toast.success(`Exam (${values.examType.toUpperCase()}) created successfully`);
              formik.resetForm();
            } else {
