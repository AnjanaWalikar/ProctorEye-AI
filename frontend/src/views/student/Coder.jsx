...
export default function Coder() {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) handleTabSwitchDetected();
    };

    const handleBlur = () => {
      if (document.hidden || document.visibilityState !== 'visible') handleTabSwitchDetected();
    };
  ...
  const runCode = async () => {
    if (tabBlocked) {
      toast.error('This exam is blocked due to tab switching. You cannot run code.');
      return;
    }
  ...
  const handleSubmit = async () => {
    if (tabBlocked) {
      toast.error('This exam is blocked due to tab switching. You cannot submit.');
      return;
    }
  ...
  return (
    <Box sx={{ p: 3, height: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
              ...
              {tabBlocked && (
                <Typography color="error" variant="body2" sx={{ mb: 1 }}>
                  Exam blocked due to tab switching. Running or submitting code is disabled.
                </Typography>
              )}
              <Box sx={{ mt: 2, display: 'flex', gap: 2 }}>
                <Button variant="contained" onClick={runCode} sx={{ minWidth: 120 }} disabled={tabBlocked}>
                  Run Code
                </Button>
                <Button
                  variant="contained"
      ...
      <Dialog open={tabWarningOpen} onClose={() => setTabWarningOpen(false)} fullWidth maxWidth="sm">
